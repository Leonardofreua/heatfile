...
class Tree:
    ...
    def __init__(self, path, parent_path, is_last, search_string) -> None:
    ...
    def make_tree(cls, root, parent=None, is_last=False, search_string=None):
        ...
        for path in childrens:
            ...
            if path.is_dir():
                ...
                if cls.__strings_incidences is not None:
    ...
    def displayable(self):
        ...
        incidences = f"({self.__strings_incidences})" if self.path.is_file() else ""
    ...
    @classmethod
    def build_tree(cls, path: Path, search_string=None):
        interable_paths = cls.make_tree(root=path, search_string=search_string)
